...
def subTestInfo = ""
    ...
    // Scenario 1: The user has not yet clocked in
    subTestInfo = " - The user has not yet clocked in"
    ...
    try {
        ...
        try {
            ...
            clickOn fxer['password'] pause(pauseMedium)
            type script.getPassword2() pause(pauseMedium)
            clickOn 'Time In' pause(pauseMedium)

            capture = new Robot().createScreenCapture(screenRect)
            ImageIO.write(capture, "bmp", new File(scrnShotDir.getAbsolutePath() + testTitle + sybTestInfo + '.bmp'))
            clickOn 'text:OK' pause(pauseMedium)
        ...
        subTestInfo = " - Already clocked in"
        ...
        clickOn 'text:Time In' pause(pauseSmall)
        clickOn fxer['clockOk'] pause(pauseMedium)

        script.logger("INFO: Taking a screenshot [AlreadyClockedIn.bmp]...\n")
        capture = new Robot().createScreenCapture(screenRect)
