...
TEST_TITLE="Transact Menu Product";
...
BASE_TOTAL_AMOUNT=45.00
PAYMENT_TENDER=45.00

# Formula provided for the computation of discount applications for SC20
VATABLE=$(printf "%.2f" $(echo "scale=4; $BASE_TOTAL_AMOUNT / 1.12" | bc))
VAT_AMT=$(printf "%.2f" $(echo "scale=4; $BASE_TOTAL_AMOUNT - $VATABLE" | bc))
...
ITEM_NAME=BBHOTDOGSPICY # Full Name of the Product, Barcode/GTIN of the Item Scanned
